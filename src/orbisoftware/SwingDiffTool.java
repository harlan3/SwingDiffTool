...
public class SwingDiffTool extends JFrame {
    ...
    private void chooseFile(boolean left) {
        // Start in the directory of the file already loaded on this side.
        // The left and right choosers intentionally use their own paths.
        Path currentPath = left ? leftPath : rightPath;
        JFileChooser fc;
        if (currentPath != null) {
            Path parent = currentPath.toAbsolutePath().normalize().getParent();
            fc = (parent != null && Files.isDirectory(parent))
                    ? new JFileChooser(parent.toFile())
                    : new JFileChooser();
        } else {
            fc = new JFileChooser();
        }
